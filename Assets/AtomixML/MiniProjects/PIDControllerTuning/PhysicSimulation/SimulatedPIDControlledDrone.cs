...
namespace Atom.MachineLearning.MiniProjects.PIDControllerTuning
{
    ...
    public class SimulatedPIDControlledDrone : MonoBehaviour, IGeneticOptimizable<NVector, NVector>
    {
        ...
        [Header("Translation")]
        [SerializeField, Range(.01f, 10f)] private float _trs_mult = 1f;
        ...
        [Header("Rotation")]
        [SerializeField, Range(.01f, 10f)] private float _rot_mult = 1f;
        ...
        [SerializeField, ReadOnly] private Vector3 _angleError;
        ...
        private void UpdatePIDParameters()
        {
            for (int i = 0; i < _translationAxisPIDFunctions.Length; i++)
            {
                _translationAxisPIDFunctions[i].SetParameters(_trs_P * _trs_mult, _trs_I * _trs_mult, _trs_D * _trs_mult, _trs_DRange);
                _translationAxisPIDFunctions[i].SetTime(Time.fixedDeltaTime, _trs_maxRecordTime);
            }

            for (int i = 0; i < _rotationAxisPIDFunctions.Length; ++i)
            {
                _rotationAxisPIDFunctions[i].SetParameters(_rot_P * _rot_mult, _rot_I * _rot_mult, _rot_D * _rot_mult, _rot_DRange);
...
*/
        ...
        private void FollowTargetMode2(Vector3 positionError, Vector3 pidVector)
        {
            ...
            if (dot > 0)
                baseThrottle = Vector4.one * pidVector.magnitude * _translationSensivity * dot;
        ...
        private void StabilizeMode3(Vector3 pidVector)
        {
            ...
            HandleDiagonalEngines(y_angle_error, ref _engineThrust);
        ...
        private void HandleDiagonalEngines(double orientation, ref Vector4 engineThrust)
        {
            Debug.DrawRay(_top_left_motor.position, transform.right * -(float)orientation, Color.blue);

            _rigidbody.AddTorque(transform.up * -(float)orientation * _translationSensivity, ForceMode.Force);
        }
        ...
        [SerializeField] private bool _useQuaternionStabilization = false;
        private void UpdateAngleMode()
        {
            ...
            if (!_useQuaternionStabilization)
            {
                // roulis / left to right on Z
                ...
                HandleDiagonalEngines(pidForce.y, ref _engineThrust);
                HandleFrontBackEngines(pidForce.x, ref _engineThrust);
                HandleLeftRightEngines(pidForce.z, ref _engineThrust);
            ...
            else
            {
                ...
                Quaternion diff = _target.rotation * Quaternion.Inverse(_rigidbody.rotation);
                Vector3 eulers = OrientTorque(diff.eulerAngles);
                eulers = transform.InverseTransformDirection(eulers);

                Debug.DrawRay(transform.position, eulers, Color.black);
                _angleError = eulers;

                var x_force = (float)_rotationAxisPIDFunctions[0].Compute(-eulers.x, 0);

                // y force is handled by a difference of rotation between opposite engines (not simulated now)
                var y_force = (float)_translationAxisPIDFunctions[1].Compute(eulers.y, 0);

                // z force is handled by a difference of rotation with left and right engines
                var z_force = (float)_rotationAxisPIDFunctions[2].Compute(eulers.z, 0);
        ...
        private void _SetRotationForce(Rigidbody rigidbody)
        {

            static float _GetSpringForce(float mass, float springTravel, float velocity, float positionSpringStrength, float positionSpringDamper)
            {
                // Find the magnitude of the dampened spring force
                return (springTravel * positionSpringStrength * mass) - (velocity * positionSpringDamper * mass);
            }

            //Find the rotation difference in eulers
            Quaternion diff = Quaternion.Inverse(rigidbody.rotation) * _target.rotation;
            Vector3 eulers = OrientTorque(diff.eulerAngles);
            ...
            /*
            //You could rescale the torque here but it will only be out of range at extreme offsets / momentarily. (Replace torque.x.y.z)
            Vector3 springTravel = OrientTorque(torque);
            */


            torque = new Vector3(
                _GetSpringForce(rigidbody.mass, torque.x, rigidbody.angularVelocity.x, _rotationSensivity, _translationSensivity),
                _GetSpringForce(rigidbody.mass, torque.y, rigidbody.angularVelocity.y, _rotationSensivity, _translationSensivity),
                _GetSpringForce(rigidbody.mass, torque.z, rigidbody.angularVelocity.z, _rotationSensivity, _translationSensivity)
            );

            _AddRotationForce(rigidbody, torque);

        }

        private void _AddRotationForce(Rigidbody rigidbody, Vector3 torque)
        {
